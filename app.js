...
// Map of original image path -> small WebP path, generated by tools/generate-media.mjs.
// Cards and the modal thumbnail strip use these; the modal stage keeps full resolution.
let thumbManifest = {};
...
// detected by extension) or objects: { "src", "type": "video", "poster" }.
const VIDEO_EXT = /\.(mp4|webm|mov|m4v|ogv)$/i;
function normalizeMedia(list) {
    return (list || []).map((m) => {
        if (typeof m === 'string') {
            return { src: m, type: VIDEO_EXT.test(m) ? 'video' : 'image', poster: '' };
        }
        if (m && typeof m === 'object' && m.src) {
            const isVideo = m.type === 'video' || VIDEO_EXT.test(m.src);
            return { src: m.src, type: isVideo ? 'video' : 'image', poster: m.poster || '' };
        }
        return null;
    }).filter(Boolean);
}
...
async function loadData() {
    try {
        ...
        if (thumbResponse && thumbResponse.ok) {
            try { thumbManifest = await thumbResponse.json(); } catch (e) { thumbManifest = {}; }
        }
